let unikernel_info (unikernel_name, info) =
  ...
  and fail_behaviour = function
    ...
    | `Restart ex ->
      ...
      `Assoc [("restart", `Null) ; ("exit-code", `List (List.map (fun e -> `Int e) els))]
  and cpuid c = `Int c
  and memory m = `Int m
  and block_devices bs =
    let block (name, dev, sec) =
      ...
      `Assoc [("name", `String name) ; ("host-device", `String dev); ("sector-size", `Int s)]
    in
  ...
  and bridges bs =
    let bridge (name, dev, mac) =
      ...
      `Assoc [("name", `String name) ; ("host-device", `String dev); ("mac", `String (Macaddr.to_string mac)) ]
    in
  ...
  `Assoc [
    ...
    ("fail-behaviour", fail_behaviour info.fail_behaviour);
    ("cpuid", cpuid info.cpuid);
    ("memory", memory info.memory);
    ("block-devices", block_devices info.block_devices);
    ("network-interfaces", bridges info.bridges);
