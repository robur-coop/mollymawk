module Make (BLOCK : Mirage_block.S) = struct
  ...
  let write_data data =
    match !disk with
    | None -> Lwt.return (Storage.error_msgf "Disk not connected")
    | Some disk -> (
        Stored_data.write disk
          (Yojson.Basic.to_string
             (Storage.t_to_json data.Storage.users data.configurations
                data.email))
        >|= function
        | Ok () -> Ok ()
        | Error we ->
  ...
  let connect block =
    Stored_data.connect block >>= fun connected_disk ->
    disk := Some connected_disk;
    read_data () >|= function
    | Error _ as e -> e
